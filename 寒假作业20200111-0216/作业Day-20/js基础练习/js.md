...
getMilliseconds();
//8.返回 1970 年 1 月 1 日至今的毫秒数。 getTime();
...
编写function parseDatetime(var datetime)方法 功能描述：传入参数的日期对象与当前日期比较：
...
function parseDatetime(y, m, d, h, min, s) {
    ...
    var seconds = date.getSeconds();
    // 2. 下面进行"粗略"判断
    if (y > year || (y == year && m > month) || (y == year && m == month && d > 31) || (y == year && m == month && d == day && h > hours) || (y == year && m == month && d == day && h == hours && min > minutes)) {
        return '日期不符合要求';
